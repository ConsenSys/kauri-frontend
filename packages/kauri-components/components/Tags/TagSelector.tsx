...
class TagSelector extends React.Component<IProps, IState> {
  ...
  public addTag(tag: ITag) {
    ...
    this.setState({ currentSelectedTags }, () =>
      this.props.onChange(currentSelectedTags)
    );
  }
  ...
  public render() {
    return (
      <Container>
        ...
        {Array.isArray(this.state.currentSelectedTags) &&
          this.state.currentSelectedTags.map(i => (
            <Tag key={i} color="white" removeTag={this.removeTag} tag={i} />
          ))}
