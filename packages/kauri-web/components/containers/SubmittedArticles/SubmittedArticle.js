...
const CategoryBadge = styled(OpenRequestCategoryBadge)`
  height: 120px;
  width: 120px;
  margin-right: 0px;
`;
const MetaDetails = styled(Header)`
  margin-top: auto;
  > :not(:first-child) {
    margin-left: 30px;
  }
...
const StatusDetails = styled(MetaDetails)`
  margin: auto 0;
  > :not(:first-child) {
    margin-left: 80px;
  }
  ...
  <SubmittedArticle>
    <SubmittedArticle.SubmittedArticleDetails>
      <SubmittedArticle.CategoryBadge
        onClick={() =>
          routeChangeAction(`/article/${article_id}/v${article_version}`)
        }
        category={category}
        theme={theme}
      >
        {category && (
          <SubmittedArticle.CategoryAvatar
            src={`/static/images/${category}/avatar.png`}
            alt="logo"
          />
        )}
      ...
      >
        <SubmittedArticle.Content type={type}>
          <SubmittedArticle.Header>
            <Link
              useAnchorTag
              route={`/article/${article_id}/v${article_version}`}
            >
              <SubmittedArticle.Subject
                href={`/article/${article_id}/v${article_version}`}
              >
                {subject}
              </SubmittedArticle.Subject>
            </Link>
          ...
          <SubmittedArticle.MetaDetails>
            ...
            <Link
              useAnchorTag
              route={`/public-profile/${user && user.user_id}`}
            >
      ...
      <Badges>
        ...
        {type !== "approval" &&
          ...
          (status !== "APPROVED" && (
            <Badge>
              <strong>
                {Array.isArray(comments) ? comments.length.toString() : 0}
              </strong>
              <span>Comments</span>
            </Badge>
          ))}
    ...
    <Divider
      style={
        typeof request_id === "string"
          ? { width: "calc(100% - 140px)", marginLeft: 140 }
          : {}
      }
    />
    {typeof request_id === "string" && (
